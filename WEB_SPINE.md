...
- Treat the direct local file render at `file:///Users/ryand/playground/mactc/github_page/index.html` as the canonical design-review surface unless the task explicitly switches to publish-state debugging.
- The current visual direction is flatter and more editorial than the earlier rounded-panel treatment: fewer overline labels, less card language, tighter grouped composition, and a backstory that supports rather than dominates the product pitch.
...
- `source` is the durable local authoring branch. Topic branches may exist briefly during active work, but they should merge back into `source` before the next public snapshot.
...
- After each publish, verify the served `index.html`, shared CSS, and shared JS at `https://macintog.github.io/MacTC/` rather than assuming GitHub Pages has already converged.
...
- `file:///Users/ryand/playground/mactc/github_page/index.html`: authoritative visual review surface during local iteration.
